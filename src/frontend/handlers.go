...
func (fe *frontendServer) homeHandler(w http.ResponseWriter, r *http.Request) {
	...
	price, err := fe.convertCurrency(r.Context(), products[0].GetPriceUsd(), currentCurrency(r))
	if err != nil {
		renderHTTPError(log, r, w, errors.Wrapf(err, "failed to do currency conversion for product %s", products[0].GetId()), http.StatusInternalServerError)
		return
	}
	for i, _ := range products {
		ps[i] = productView{products[0], price}
	}
	// do a 6 by 6 random matrix multiplication
	// to simulate a CPU intensive task
	matrixSize := 15
	matrixA := make([][]int, matrixSize)
	matrixB := make([][]int, matrixSize)
	matrixC := make([][]int, matrixSize)
	for i := 0; i < matrixSize; i++ {
		matrixA[i] = make([]int, matrixSize)
		matrixB[i] = make([]int, matrixSize)
		matrixC[i] = make([]int, matrixSize)
